  ...
  const [plants, setPlants] = useState([
    {
      ...
      timer: 2,
      title: "المريخ",
      content: "ثابت الجاذبيه = 3.73 متر/ثانيه2",
    },
    {
      name: "moon",
      id: "5",
      ...
      timer: 3,
      title: "القمر",
      content: "ثابت الجاذبيه = 1.62 متر/ثانيه2  ",
    },
    {
      name: "earth",
      id: "6",
      ...
      timer: 1,
      title: "الارض",
      content: "ثابت الجاذبيه = 9.81 متر/ثانيه2",
    },
  ]);
          ...
          <div className="mainBalls flex flex-col px-2 pt-2 pb-8 items-center gap-[110px] rounded-lg bg-[#5484FF] w-[138px]">
            <div className="balls flex flex-col gap-1 w-full">
              {balls.map((ball) => (
                <div
                  key={ball.id}
                  className=" bg-[#466fd8] rounded-md flex items-center flex-col pb-1"
                >
                  ...
                  <p className="mt-7 text-white font-extrabold">
                    {ball.id} كيلو جرام
                  </p>
                </div>
              ))}
            ...
            <div className="images flex rounded-l p-3">
              {plants.map((plant) => (
                ...
                >
                  ...
                  />
                  {balls
                    .filter((ball) => ball.container === plant.name)
                    .map((ball) => {
                      var myOverBallId;
                      if (ball.name === ball.activity) {
                      ...
                      return (
                        <Draggable
                          ...
                          transition={
                            myOverBallId
                              ? `top ${plant.timer * ball.weight}s ease-in-out`
                              : ""
                          }
                          className={`z-10 transition-all duration-75 ease-linear cursor-pointer hover:translate-110 hover:scale-110`}
                        >
                          <Image
                            src={ball.src}
                            alt={ball.name}
                      ...
                      activeBallId === ball.name ? (
                        <div key={ball.id}>
                          <Image
                            ...
                            width={94}
                            height={94}
                            className="z-10 transition-all duration-75 ease-linear cursor-pointer hover:translate-110"
                          />
                        </div>
                      ) : null
                    )
